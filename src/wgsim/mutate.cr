...
module Wgsim
  class Mutate
    ...
    def initialize(@option : Option)
      ...
      @mutated_fasta = option.mutated_fasta ||
                       raise(ArgumentError.new("Output FASTA file is required"))
      @mutation_event_log = option.mutation_event_log ||
                            raise(ArgumentError.new("Output mutation file is required"))
      option.validate!
    ...
    private def process_sequence(
      ...
      fasta_writer : Fastx::Fasta::Writer,
      mutation_io : IO,
    )
      if option.ploidy == 1
        simulate_and_output_sequence(
    ...
    private def simulate_and_output_sequence(
      reference_name : String,
      reference_sequence : Slice(UInt8),
      copy_number = nil,
      *,
      fasta_writer : Fastx::Fasta::Writer,
      mutation_io = STDERR,
